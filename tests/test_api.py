...
requires_db = pytest.mark.skipif(
    not os.getenv("DATABASE_URL"), reason="DATABASE_URL not set — skipping DB tests"
)
...
def logged_in(client):
    ...
    """
    client.post(
        "/auth/register", json={"username": VALID_USERNAME, "password": VALID_PASSWORD}
    )

    response = client.post(
        "/auth/login", json={"username": VALID_USERNAME, "password": VALID_PASSWORD}
    )
...
class TestAuthEndpoints:
    ...
    def test_register_rejects_short_password(self, client):
        response = client.post(
            "/auth/register", json={"username": "validuser", "password": "Short1@"}
        )
        assert response.status_code == 400
    ...
    def test_register_rejects_password_without_uppercase(self, client):
        response = client.post(
            "/auth/register",
            json={"username": "validuser", "password": "alllowercase1@3$567890"},
        )
        assert response.status_code == 400
    ...
    def test_register_rejects_password_without_special_char(self, client):
        response = client.post(
            "/auth/register",
            json={"username": "validuser", "password": "NoSpecialChar12345678"},
        )
        assert response.status_code == 400
    ...
    def test_register_rejects_invalid_username(self, client):
        response = client.post(
            "/auth/register", json={"username": "ab", "password": VALID_PASSWORD}
        )
        assert response.status_code == 400
    ...
    def test_register_success(self, client):
        response = client.post(
            "/auth/register",
            json={"username": "newuser_reg", "password": VALID_PASSWORD},
        )
    ...
    def test_login_success(self, client):
        client.post(
            "/auth/register",
            json={"username": "newuser_login", "password": VALID_PASSWORD},
        )
        response = client.post(
            "/auth/login",
            json={"username": "newuser_login", "password": VALID_PASSWORD},
    ...
    def test_login_wrong_password(self, client):
        client.post(
            "/auth/register",
            json={"username": "newuser_wrong", "password": VALID_PASSWORD},
        )
        response = client.post(
            "/auth/login",
            json={"username": "newuser_wrong", "password": "WrongPass1@3$567890XX"},
        )
...
class TestEntriesEndpoints:
    ...
    def test_create_entry_without_auth_returns_401(self, client):
        response = client.post(
            "/entries/",
            json={"name": "test", "ciphertext": "xyz", "iv": "abc", "tag": "def"},
        )
        assert response.status_code == 401
    ...
    def test_cannot_access_another_users_entry(self, client):
        client.post(
            "/auth/register",
            json={"username": "user_one_x", "password": VALID_PASSWORD},
        )
        client.post(
            "/auth/register",
            json={"username": "user_two_x", "password": VALID_PASSWORD},
        )

        token1 = client.post(
            "/auth/login", json={"username": "user_one_x", "password": VALID_PASSWORD}
        ).json()["accessToken"]
        ...
        token2 = client.post(
            "/auth/login", json={"username": "user_two_x", "password": VALID_PASSWORD}
        ).json()["accessToken"]

        response = client.get(
            f"/entries/{entry_id}", headers={"Authorization": f"Bearer {token2}"}
        )
